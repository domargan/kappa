...
void preload_states(Digraph* digraph,
  ...
  if (fs) {
    std::cout
        << "\n-----------------------------------------------------------------"
           "------------------------"
           "\n[START]\t\tPreloading core vertex states with entries from file "
    ...
    while (getline(fs, line)) {
      ...
      if (line_counter >= beggining_line && line_counter <= end_line) {
        ...
        digraph->set_state(static_cast<vertex_id_t&&>(stoi(vertex_id)),
                           static_cast<state_t&&>(stod(state)));
