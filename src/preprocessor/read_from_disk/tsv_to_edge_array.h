...
typedef std::vector<std::vector<uint32_t>> raw_edge_array_t;
...
uint32_t unique_vertex_count(raw_edge_array_t&);
std::set<uint32_t> extract_vertices(raw_edge_array_t&);
