...
void Scheduler::init_threads(const std::uint32_t num_threads,
...
void Scheduler::init_numa_node(const uint node_no, const uint no_of_threads) {
  try {
    for (uint i = 0; i < no_of_threads; ++i) {
      ...
      std::cout << "[INFO]\t\tThread pinned to CPU " << node_no + (i * 4)
                << std::endl;
    }
  } catch (...) {
    destroy();
...
void Scheduler::submit(Task* task) {
  ...
  // TODO: Remove this block for measurements
  {
      ...
      else if (task->task_type == ON_UPDATE) { task_type = 1; } // On add/remove
  edge else if (task->task_type == UPDATE) { task_type = 2; } // Insert/delete
  edge

      if (task_counter % 100 == 0) { // or 1000 when having more inputs
          fs << std::chrono::duration<float>(std::chrono::steady_clock::now() -
  tp_start).count() << " " // Real time
...
void Scheduler::worker(void) {
  while (!done) {
    if (!paused) {  // Try to use the blocking feature of the que instead of
                    // this, e.g. while(task_queue.try_pop(...))
      Task* task;
      // task_queue.pop(task, active);
      if (task_queue.try_pop(task, active)) {
        // std::cout << task->task_type << " " << task->timestamp_logical <<  "
        ...
        switch (task->task_type) {
            case ON_ACTIVATE:                    // On activate (basically
        "compute") task->g->set_vertex_on_activate_ts(task->v,
        task->timestamp_logical);

                break;
            case ON_UPDATE:                      // What to do after
        adding/removing and edge (e.g. run "activate")
                task->g->set_vertex_on_update_ts(task->src,
        task->timestamp_logical); task->g->set_vertex_on_update_ts(task->dst,
        task->timestamp_logical);
            ...
            case UPDATE:                    // Just removing and adding edges
                ...
                //std::cout << task->g->get_vertex_update_ts(task->v) <<
        std::endl; task->g->set_vertex_update_ts(task->src,
        task->timestamp_logical); task->g->set_vertex_update_ts(task->dst,
        task->timestamp_logical);
                // TODO: Introduce blocking logic for correctness
