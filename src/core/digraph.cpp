...
// TODO: Implement vertex iterator (instead of iterating until
// vertex_index.size())
// TODO: Do something smarter about getting size; do not call
// vertex_index.size() all the time
// TODO: For every iteration of vertices from 0 to vertex_index.size() check if
// the vertex exists before any operations
// TODO: Remove all calls to vertex_index.size() and replace them with a
// variable
...
Digraph::Digraph(graph_size_t v_num,
  ...
  std::cout << "\n-------------------------------------------------------------"
               "----------------------------"
               "\n[START]\t\tDigraph constructor called..."
            << std::endl;
  ...
  for (int i = 0; i < max_vertex_allocations; i++) {
    ...
    dv.on_activate_ts = 0;
    // dv.component_label =
    // std::numeric_limits<components_number_t>::infinity();
  ...
  std::cout << "[END]\t\tDigraph structure initialized with " << v_num
            << " vertex entries." << std::endl;
}
...
bool Digraph::has_edge(vertex_id_t src_v, vertex_id_t dst_v) {
  // std::find() has a worst-case time of O(n) in the distance between first and
  // last.

  // TODO: Remove duplicated neighbors vectors and implement a new way of
  // checking for existance of the edges that are in the opposite direction of
  // those represented by neighbor vectors
  return std::find(topology[src_v].out_neighbors.begin(),
                   topology[src_v].out_neighbors.end(),
                   dst_v) != topology[src_v].out_neighbors.end();
}
...
void Digraph::remove_edge(vertex_id_t src_v, vertex_id_t dst_v) {
  if (has_edge(src_v, dst_v)) {
    ...
    topology[src_v].out_neighbors.erase(
        std::remove(topology[src_v].out_neighbors.begin(),
                    topology[src_v].out_neighbors.end(), dst_v),
        topology[src_v].out_neighbors.end());

    topology[dst_v].in_neighbors.erase(
        std::remove(topology[dst_v].in_neighbors.begin(),
                    topology[dst_v].in_neighbors.end(), src_v),
...
neighbors_vector_t Digraph::get_in_out_neighborhood(vertex_id_t v) {
  ...
  std::copy(topology[v].out_neighbors.begin(), topology[v].out_neighbors.end(),
