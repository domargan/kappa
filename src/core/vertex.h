...
typedef struct Dvertex {
    ...
    timestamp_logical_t update_ts; // Timeestamp of the last UPDATE task that added or removed an adge to the vertex
    timestamp_logical_t on_update_ts; // Timestamp of the last ON_UPDATE task that touched the vertex
    timestamp_logical_t on_activate_ts; // Timestamp of the last ON_ACTIVATE task that touched the vertex
    bool scheduled;
} Dvertex;
