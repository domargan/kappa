...
// TODO: Check which get() methods should return pointers and which should
// return values
...
// TODO: Change long to int in dynamic_bitset allocation... It does not work for
// some reason :)
typedef boost::dynamic_bitset<unsigned long, std::allocator<unsigned long>>
    vertex_bitset_t;
...
class Digraph {
 private:
  graph_size_t
      max_vertex_allocations;  // Set a limit for the maximum possible number of
                               // vertices to fit in the graph.

  digraph_vector_t topology;     // Graph topology data + some metadata.
  vertex_bitset_t vertex_index;  // 1 if the vertex is present, 0 if the vertex
                                 // is not in the graph.

  state_vector_t states;

  vertex_bitset_t visited_verts;

  components_number_t cc_count;
 ...
 public:
  ...
  digraph_vector_t
  get_digraph_vector();  // Return the graph_vector_t data structure.
  vertex_bitset_t
  get_vertex_index();  // Return index of vertices present in the graph.
  ...
  void add_edge_populate(vertex_id_t,
                         vertex_id_t);  // Same as above but optimised to
                                        // prepopulate the graph with edges.
  void remove_edge(vertex_id_t,
                   vertex_id_t);  // Remove the edge between two vertices.
  bool has_edge(vertex_id_t,
                vertex_id_t);  // Return true of the edge is in the graph.

  void print_edges();  // (use just for testing)

  neighbors_vector_t get_in_neighborhood(
      vertex_id_t);  // Return the vector of incoming neighbors of a vertex.
  neighbors_vector_t get_out_neighborhood(
      vertex_id_t);  // Return the vector of outcoming neighbors of a vertex.
  ...
  graph_size_t get_out_degree(
      vertex_id_t);                      // Return the out-degree of a vertex.
  ...
  void
  count_order();  // Count and set the number of vertices. OBSOLETE FUNCTION
  graph_size_t get_order();  // Return the number of vertices.
  graph_size_t
  get_max_order();  // Return the number of pre-allocated vertex spaces.
  ...
  void increment_cc_count();
  void decrement_cc_count();
  components_number_t get_cc_count();
  void set_component_label(vertex_id_t, components_number_t);
  components_number_t get_component_label(vertex_id_t);

  void reset_cc_count_and_labels();
};
